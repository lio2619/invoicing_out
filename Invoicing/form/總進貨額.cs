...
namespace Invoicing
{
    public partial class 總進貨額 : Form
    {
        ...
        private async void button1_Click(object sender, EventArgs e)
        {
            ...
            string SQL = "select 客戶, 單子, sum(總金額) as 金額 from 總單子_客戶 where 日期 between @first_day and @second_day and 單子 in('進貨退出單', '進貨單')" +
            ...
            SQL = "select sum(總金額) as 金額 from 總單子_客戶 where 日期 between @first_day and @second_day and 單子 = '進貨退出單'";
            dt = await con.searchDataTable(SQL, new
            {
                first_day = dateTimePicker1.Value.ToString("yyyyMMdd"),
                second_day = dateTimePicker2.Value.ToString("yyyyMMdd")
            });
            SQL = "select sum(總金額) as 金額 from 總單子_客戶 where 日期 between @first_day and @second_day and 單子 = '進貨單'";
            DataTable DT = await con.searchDataTable(SQL, new
            {
                first_day = dateTimePicker1.Value.ToString("yyyyMMdd"),
                second_day = dateTimePicker2.Value.ToString("yyyyMMdd")
