        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (dialog.ShowDialog() == DialogResult.OK)
            {
                ...
                try
                {
                    using (OleDbConnection connect = new OleDbConnection(connect_string))
                    {
                        ...
                        //SheetName = "復原_工作表1$";
                        //依序為：國碼、品名、數量、單位、單價
                        sql = "select F1, F2, F3, F4, F5 from [" + SheetName + "A16:H] where IsNumeric(F1) = True";
                ...
                {
                    MessageBox.Show("讀取 Excel 失敗：檔案缺失");
                }
            }
            SQLConnect con = new SQLConnect();
            ...
            for (int i = 0; i < DT.Rows.Count; i++)
            {
                ...
                if (grid != null)
                {
                    ...
                    {
                        cost = (double.Parse(grid.Rows[0]["標準售價"].ToString()) * double.Parse(DT.Rows[i]["F3"].ToString())).ToString();
                    ...
                    else if (string.IsNullOrEmpty(DT.Rows[i]["F2"].ToString()))
                    {
                        break;
                    }
                    else
                ...
                if (grid.Rows.Count == 0)
                {
                    dataGridView1.Rows[i].Cells[1].Value = DT.Rows[i]["F2"].ToString();
                    dataGridView1.Rows[i].Cells[2].Value = DT.Rows[i]["F3"].ToString();
                    dataGridView1.Rows[i].Cells[3].Value = DT.Rows[i]["F4"].ToString();
                    workRow["品名"] = DT.Rows[i]["F2"].ToString();
                    workRow["基本單位"] = DT.Rows[i]["F4"].ToString();
                    workRow["數量"] = DT.Rows[i]["F3"].ToString();
                    string SQL = "insert into 貨品主檔 (貨品編號, 品名, 基本單位) values(@store," +
                        " @name, @unit)";
                    con.execute(SQL, new
                    {
                        store = DT.Rows[i]["F1"].ToString(),
                        name = DT.Rows[i]["F2"].ToString(),
                        unit = DT.Rows[i]["F4"].ToString()
                    });
                }
                else
                {
