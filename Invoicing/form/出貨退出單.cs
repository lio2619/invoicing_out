...
namespace Invoicing
{
    public partial class 出貨退出單 : Form
    {
        ...
        private async void create_excel(string date, string file_name, string number)
        {
            ...
            file = new FileInfo(@"../單子/" + date + "/出貨退出單/" + file_name.Split('(')[0] + "_" + number + ".xlsx");
        ...
        private void dataGridView1_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            {
                模糊查詢 form2 = new 模糊查詢(e.RowIndex, "出貨退出單");
                form2.Owner = this;
                form2.Show();
            }
        }
        ...
        private async void button4_Click(object sender, EventArgs e)
        {
            ...
            string path = "../單子/" + date + "/出貨退出單/";
            if (!Directory.Exists(path))
            {
                Directory.CreateDirectory(path);
            }
            SQLConnect con = new SQLConnect();
            string SQL = "select 單子編號 from 總單子_客戶 where 日期=@time and 客戶=@store_name and 單子='出貨退出單' " +
            ...
            string file_name = "出貨退出單_" + dateTimePicker1.Value.ToString("yyyy-MM-dd") + "_" + comboBox1.Text.ToString();
