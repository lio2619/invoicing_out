    ...
    public partial class 訂貨單 : Form
    {
        ...
        private async void dataGridView1_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.ColumnIndex == 0)
            {
                if (dataGridView1.Rows[e.RowIndex].Cells[0].Value != null)
                {
                    ...
                    string sql = "select 品名, 基本單位, 標準售價 from 貨品主檔 where 貨品編號=@find";
                    ...
                    if (dt != null)
                    {
                        if (dt.Rows.Count > 0)
                        {
                            ...
                            dataGridView1.Rows[e.RowIndex].Cells[4].Value = dt.Rows[0]["標準售價"].ToString();
                        }
                    }
        ...
        private async void printDocument1_PrintPage(object sender, System.Drawing.Printing.PrintPageEventArgs e)
        {
            ...
            try
            {
                ...
                SQL = "select 單子編號 from 總單子_客戶 where 日期=@time and 客戶=@store_name and 單子='訂貨單' " +
                ...
                if (dt != null)
                {
                    if (dt.Rows.Count > 0)
                    {
                        form.print_header_setting(e, "訂貨單", store_name, dateTimePicker1.Text.ToString(), dateTimePicker1.Value.ToString("yyyyMMdd") + number.ToString("000"),
            ...
            for (; row < dataGridView1.RowCount - 1;)    //40就要換頁
            {
                ...
                {
                    form.print_inside_setting(e, dataGridView1.Rows[row].Cells[j].Value?.ToString() ?? "", high, j, row, "訂貨單");
                }
        ...
        private async void button2_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                    sql = "insert into 總單子_客戶 (日期, 時間, 客戶, 單子, 備註 ,總金額, 單子編號, 刪除) values (@day, @time, @store_name, '訂貨單'," +
        ...
        private async void create_excel(string date, string file_name, string number)
        {
            ...
            file = new FileInfo(@"../單子/" + date + "/訂貨單/" + file_name.Split('(')[0] + "_" + number + ".xlsx");
        ...
        private void dataGridView1_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            {
                模糊查詢 form2 = new 模糊查詢(e.RowIndex, "訂貨單");
                form2.Owner = this;
                form2.Show();
            }
        }
        ...
        private async void button5_Click(object sender, EventArgs e)
        {
            ...
            string path = "../單子/" + date + "/訂貨單/";
            if (!Directory.Exists(path))
            {
                Directory.CreateDirectory(path);
            }
            SQLConnect con = new SQLConnect();
            string SQL = "select 單子編號 from 總單子_客戶 where 日期=@time and 客戶=@store_name and 單子='訂貨單' " +
                        "and 備註=@remark and 總金額=@total_cost";
            DataTable dt_1 = await con.searchDataTable(SQL,
                new
                {
            ...
            int number = int.Parse(dt_1.Rows[0]["單子編號"].ToString());
            string file_name = "訂貨單_" + dateTimePicker1.Value.ToString("yyyy-MM-dd") + "_" + comboBox1.Text.ToString();
